...
pred, h_n, h_c = rnn_cpu(var_dataX)
batch, time_step, hidden_size = np.shape(pred)
loss = loss_func(pred.reshape(batch, -1), torch.from_numpy(dataY[train_size:].reshape(-1, 1, 1)).reshape(batch, -1))
